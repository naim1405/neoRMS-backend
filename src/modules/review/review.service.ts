...
const getOrderBy = (sortBy?: string, sortOrder?: string) => {
	...
	const finalSortBy = sortBy && allowedSortBy.has(sortBy) ? sortBy : 'createdAt';
...
const managementGetAllReviews = async (
	...
	const whereClause = {
		...
		...(filters.menuProductId ? { menuProductId: filters.menuProductId } : {}),
		...(orderMenuItemIds ? { menuProductId: { in: orderMenuItemIds } } : {}),
		menuProduct: {
			tenantId,
			isDeleted: false,
		},
...
const managementGetReviewsByOrder = async (orderId: string, tenantId: string) => {
	const order = await prisma.order.findUnique({
		where: {
			id: orderId,
			tenantId,
